...
window.onload = _ => {
    // load & set json data
    fetch(new Request(`./assets/cv/json/cv.json`, {
        method: `GET`,
        headers: new Headers(),
        mode: `cors`,
        cache: `default`
        ...
        for (const key in _data) _html += `
            ...
            <div class="div_des"><p class="p_title">${_data[key].title}</p><p class="p_des">${_data[key].description}</p></div>
            </div>
            </a>
            </li>`;
...
window.matchMedia(`(prefers-color-scheme: dark)`).addEventListener(`change`, e => _setDarkMode(e.matches));
